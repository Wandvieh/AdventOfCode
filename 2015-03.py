...
Starts at x=0, y=0
- Sets counter at current house to True
- Moves
    If < then to 
    Then sets the counter there to True
In the end, counts how many True's there are
...
for i in range(len(input)):
    if (x1, y1) not in visited: visited.append((x1,y1))
    if (x2,y2) not in visited: visited.append((x2,y2))
    if i%2 == 0:
        if input[i] == '>': x1 += 1
        elif input[i] == '<': x1 -= 1
        elif input[i] == '^': y1 += 1
